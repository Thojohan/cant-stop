export function CreateDot({
  ...
  const matchedValue = commitedArray
    .filter((el) => Object.keys(el)[0] === `${value}`)
    .reduce((acc, el) => {
      if (el.length === 0) return acc;
      const [[key, val]] = Object.entries(el);
      let temp;

      if (+val >= position[+value - 2].length && isTemp === false) {
        const allFilledArray = Array(position[+value - 2].length)
          .fill(null)
          .map((_, i) => {
            return { [value]: i + 1 };
          });
  ...
  return matchedValue.map((el, i) => (
    <span
      ...
      className="dot"
      style={{
        marginTop: `${position[+Object.keys(el) - 2][+Object.values(el) - 1]}%`,
